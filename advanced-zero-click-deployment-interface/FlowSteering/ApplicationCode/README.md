...
If you encounter any issues while running the GenAI EcoSystem, here are some common troubleshooting steps:

1. **Network Issues**: Ensure that all machines (Email Server, LLaVa Server, and End User Clients) are on the same network and can communicate with each other through the specified ports.

2. **GPU Compatibility**: Verify that the machine running the LLaVa Server has a compatible GPU and the necessary drivers installed.

3. **File Paths**: Double-check the file paths specified in the configuration files to ensure they are correct and accessible.

4. **Dependencies**: Make sure all required dependencies are installed. You can refer to the `requirements.txt` file for a list of dependencies.

5. **Logs**: Check the logs generated by the Email Server, LLaVa Server, and End User Clients for any error messages or warnings that can help diagnose the issue.

## Examples

Here are some examples of how to use the GenAI EcoSystem:

### Example 1: Sending an Email

1. Start the Email Server and LLaVa Server as described in the "Running the GenAI EcoSystem" section.
2. Run the End User Client and Attacker Client as described in the respective sections.
3. The Attacker Client will send a malicious email to the End User Clients.
4. The End User Clients will receive the email, send it to the LLaVa Server for classification, and take appropriate actions based on the classification.

### Example 2: Checking the Inbox

1. Start the Email Server and LLaVa Server as described in the "Running the GenAI EcoSystem" section.
2. Run the End User Client as described in the respective section.
3. The End User Client will periodically check the inbox for new emails and display them in a pop-up window.

### Example 3: Forwarding an Email

1. Start the Email Server and LLaVa Server as described in the "Running the GenAI EcoSystem" section.
2. Run the End User Client as described in the respective section.
3. When the End User Client receives an email classified as "forward" by the LLaVa Server, it will forward the email to the specified recipients.

## Additional Information

For more information on running the GenAI EcoSystem, including detailed usage instructions and advanced configuration options, please refer to the `README.md` file in the root directory.
